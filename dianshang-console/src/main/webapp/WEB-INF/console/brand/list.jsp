...
<div class="body-box">
    ...
    <table id="tableList" cellspacing="1" cellpadding="0" border="0" width="100%" class="pn-ltable">
        ...
        <c:forEach items="${pageBrand.result}" var="brand">
            ...
            <tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'" onmouseover="this.bgColor='#eeeeee'">
                ...
                <td align="center">
                    <a class="pn-opt" href="showedit.do?brandId=${brand.id}">修改</a> | <a class="pn-opt" onclick="optDelete('${name}','${isDisplay}','${pageBrand.pageNum}');" href="#">删除</a>
                </td>
            </tr>
    ...
    <div class="page pb15">
			<span class="r inb_a page_b"> <a href="list.do?name=${name}&isDisplay=${isDisplay}&pageNum=1"><font
                    size="2">首页</font></a> <c:if test="${pageBrand.pageNum<=1}">
                <font size="2">上一页</font>
            </c:if> <c:if test="${pageBrand.pageNum>1}">
                <a href="list.do?name=${name}&isDisplay=${isDisplay}&pageNum=${pageBrand.pageNum-1}"><font
                        size="2">上一页</font></a>
            </c:if> &nbsp;
             ...
             <c:forEach begin="${begin}" end="${end}" var="ps">
              ...
              <c:if test="${pageBrand.pageNum>=pageBrand.pages}">
                ...
                </c:if> <a href="list.do?name=${name}&isDisplay=${isDisplay}&pageNum=${pageBrand.pages}"><font size="2">尾页</font></a> 共<var>${pageBrand.pages}</var>页 到第 <input
                        type="text" size="3" id="PAGENO"/>页 <input type="button" onclick="javascript:window.location.href = '/product/list.do?&amp;isShow=0&amp;pageNo=' + $('#PAGENO').val() " value="确定" class="hand btn60x20" id="skip"/>
			</span>

    </div>
    </form>
    <div style="margin-top:15px;"><input class="del-button" type="button" value="删除" onclick="optDelete('${name}','${brand.isDisplay}','${pageBrand.pageNum}');" /></div>
</div>
...
<script>
    ...
    var flag = true;
    //全选全不选功能
    function checkAllOrNotBox(checked) {
        if (flag) {
    ...
    function optDelete(name, isDisplay, pageNum) {
        ...
        //开始删除 	 提交表单
        $("#formList")[0].action = 'doDelete.do?name=' + name + '&isDisplay='
                + 1 + '&pageNum=' + pageNum;
        $("#formList").submit();
    }
</script>
